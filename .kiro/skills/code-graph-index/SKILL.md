...
1. A **structural graph** — files, classes, functions, imports, and call edges saved as JSON.
2. A **FAISS semantic index** — chunk-level vector search over source code.
...
### 2. Build the FAISS semantic index
...
| Tool | Purpose |
|---|---|
| `search_code_chunks(query, top_k?)` | Semantic chunk search — returns `{path, start_line, end_line, preview, score}`. |

Combine both: use FAISS to find candidate chunks, then `get_neighbors` to widen context along call edges — the graph keeps the agent grounded, FAISS keeps it discoverable.
