...
description: |
  ...
  Use when users request "comps", "comparable company analysis", "peer comparison", "trading multiples", or "valuation benchmarking".
...
1. **FIRST: User-provided data** - If the user provides financials, use them as primary source
2. **SECOND: MCP data sources** - If financial data MCP servers are available (S&P, FactSet, Daloopa), use them for verified data
3. **THIRD: Web search + SEC EDGAR** - Search for 10-K/10-Q filings, Yahoo Finance, Google Finance for current prices and financial data
4. **FOURTH: Bloomberg / institutional sources** - If available

**Always cite your data sources** with specific document references, dates, and URLs when possible.
