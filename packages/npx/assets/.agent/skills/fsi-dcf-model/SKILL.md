...
name: fsi-dcf-model
description: |
  Institutional-quality DCF (Discounted Cash Flow) model creation for equity valuation. Retrieves financial data from SEC filings, analyst reports, and web searches. Builds comprehensive cash flow projections with proper WACC calculations, performs sensitivity analysis, and outputs professional Excel models with executive summaries.
  Use when users need to value a company using DCF methodology, request intrinsic value analysis, or ask for detailed financial modeling with growth projections and terminal value calculations.
  Triggers: "DCF", "discounted cash flow", "intrinsic value", "equity valuation", "WACC", "terminal value", "valuation model".
---
...
- Use all information provided by the user, web search, and any available data sources for financial data retrieval.
- If MCP data servers are available (e.g., financial data providers), prefer them. Otherwise, fall back to web search and SEC EDGAR filings.

## Critical Constraints - Read These First

These constraints apply throughout all DCF model building. Review before starting:

**Environment: Python/openpyxl (standalone .xlsx generation):**
- Use Python with openpyxl to generate standalone .xlsx files
- Write formula strings: `ws["D20"] = "=D19*(1+$B$8)"` — NOT computed values
- All principles (formula strings, cell comments, section checkpoints, sensitivity table loops) apply throughout
...
Fetch data from user-provided data, web search, and SEC filings.

**Data Sources Priority:**
1. **User-Provided Data** - Historical financials from their research
2. **Web Search/Fetch** - Current prices, beta, debt and cash from Yahoo Finance, SEC EDGAR, etc.
3. **MCP Servers** (if configured) - Structured financial data from providers
